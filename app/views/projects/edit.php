...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h2 mb-0 text-light">Edit Proyek</h1>
        <p class="text-muted mb-0">Perbarui informasi proyek</p>
    </div>
    <a href="index.php?url=projects/view/<?php echo $project['id']; ?>" class="btn btn-outline-secondary">
                    ...
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" class="form-control" id="name" name="name" 
                                   value="<?php echo htmlspecialchars($_POST['name'] ?? $project['name']); ?>" required>
                        </div>
                    </div>

                    <div class="mb-3">
                        <label for="description" class="form-label">Deskripsi <span class="text-danger">*</span></label>
                        <textarea class="form-control" id="description" name="description" rows="4" required><?php echo htmlspecialchars($_POST['description'] ?? $project['description']); ?></textarea>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="client_name" name="client_name" placeholder="Nama klien (opsional)"
                                   value="<?php echo htmlspecialchars($_POST['client_name'] ?? ($clientName ?? '')); ?>">
                        ...
                        <div class="col-md-12 mb-3">
                                ...
                                <div class="card-body py-2">
                                    <div class="d-flex align-items-center">
                                        <div class="flex-grow-1">
                                            <div class="fw-medium"><?php echo htmlspecialchars($projectManager['full_name'] ?? 'Unknown User'); ?></div>
                                            <small class="text-muted">Pembuat Proyek (<?php echo ucfirst(str_replace('_', ' ', $projectManager['role'] ?? 'unknown')); ?>)</small>
                                        </div>
                            ...
                            <small class="text-muted d-block mt-1">Pembuat proyek tidak dapat diubah.</small>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" class="form-control" id="start_date" name="start_date" 
                                   value="<?php echo htmlspecialchars($_POST['start_date'] ?? $project['start_date']); ?>" required>
                        </div>
                        
                        <div class="col-md-6 mb-3">
                            <label for="end_date" class="form-label">Tanggal Selesai <span class="text-danger">*</span></label>
                            <input type="date" class="form-control" id="end_date" name="end_date" 
                                   value="<?php echo htmlspecialchars($_POST['end_date'] ?? $project['end_date']); ?>" required>
                        </div>
                    </div>
    ...
    <div class="col-lg-4">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    <h6 class="text-primary">
                        <i class="fas fa-lightbulb me-2"></i>Panduan Pembaruan
                    </h6>
                    <ul class="small text-muted mb-0">
                        <li>Tinjau tanggal proyek sebelum memperbarui</li>
                        <li>Pertimbangkan dampak pada anggota tim</li>
                        <li>Perbarui status berdasarkan progress aktual</li>
                        <li>Beritahu stakeholder jika ada perubahan besar</li>
                    </ul>
                </div>

                <div>
                    <h6 class="text-primary">
                        <i class="fas fa-users me-2"></i>Hak Akses
                    </h6>
                    <small class="text-muted">
                        Hanya Super Admin, Admin, dan Manajer Proyek yang dapat mengedit informasi proyek. 
                        Perubahan akan dicatat untuk keperluan audit.
                    </small>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Existing team members data from PHP
    const existingTeamMembers = <?php echo json_encode($existingTeamMembers ?? []); ?>;
    
    // Load existing team members
    if (existingTeamMembers && existingTeamMembers.length > 0) {
        existingTeamMembers.forEach(member => {
            addTeamMember(member.user_id, member.role_in_project);
        });
    } else {
        // Add initial team member if no existing members
        addTeamMember();
    }
        ...
        function updateMemberOptions() {
            ...
            if (selectedRole) {
                ...
                if (selectedRole === 'project_manager') {
                    ...
                    filteredUsers = availableUsers.filter(user => user.role === 'project_manager');
                } else {
                    // For other roles, show users with matching role
                    filteredUsers = availableUsers.filter(user => user.role === selectedRole);
                }
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        const hasProjectManager = Array.from(teamMembers).some((select, index) => {
            const roleSelect = teamContainer.querySelectorAll('.team-role-select')[index];
            return select.value && roleSelect.value === 'project_manager';
        });
        
        if (!hasProjectManager) {
            e.preventDefault();
            alert('Setidaknya satu anggota tim harus ditetapkan sebagai Manajer Proyek.');
            return false;
        }
    });
});
