...
<div class="d-flex justify-content-between align-items-center mb-4 d-print-none">
    <div>
        <h1 class="h2 mb-0 text-dark">Laporan Proyek</h1>
        <p class="text-muted mb-0">Ringkasan proyek, progress, dan penanggung jawab</p>
    </div>
...
<div class="card">
    <div class="card-header bg-light border-bottom">
        <h5 class="card-title mb-0 text-dark">
            <i class="fas fa-project-diagram me-2 text-muted"></i>Daftar Proyek
        </h5>
    </div>
    <div class="card-body">
        <?php if (empty($projects)): ?>
            <div class="text-center text-muted">Tidak ada data proyek.</div>
        <?php else: ?>
        <!-- Screen View Table -->
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo number_format((float)($p['progress'] ?? 0), 0); ?>%</td>
                        <td><?php echo !empty($p['start_date']) ? date('Y-m-d', strtotime($p['start_date'])) : '-'; ?></td>
                        <td><?php echo !empty($p['end_date']) ? date('Y-m-d', strtotime($p['end_date'])) : '-'; ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        ...
        <div class="d-none d-print-block">
            ...
            <h3 class="text-center mb-4 fw-bold" style="color: #fff;">Laporan Proyek</h3>

            <table class="table table-bordered border-dark">
                <thead>
                    <tr class="table-light border-dark">
                        <th class="border-dark text-center" style="width: 5%;">No</th>
                        <th class="border-dark">Nama Proyek</th>
                        <th class="border-dark">Klien</th>
                        <th class="border-dark">Manajer</th>
                        <th class="border-dark text-center">Status</th>
                        <th class="border-dark text-center">Progress</th>
                        <th class="border-dark text-center">Tgl Mulai</th>
                        <th class="border-dark text-center">Tgl Selesai</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr class="border-dark">
                        ...
                        <td class="border-dark fw-bold"><?php echo htmlspecialchars($p['name'] ?? ''); ?></td>
                        <td class="border-dark"><?php echo htmlspecialchars($p['client_name'] ?? '-'); ?></td>
                        <td class="border-dark"><?php echo htmlspecialchars($p['manager_name'] ?? '-'); ?></td>
                        <td class="border-dark text-center"><?php echo ucfirst(str_replace('_',' ', $p['status'] ?? '')); ?></td>
                        <td class="border-dark text-center"><?php echo number_format((float)($p['progress'] ?? 0), 0); ?>%</td>
                        <td class="border-dark text-center"><?php echo !empty($p['start_date']) ? date('d/m/Y', strtotime($p['start_date'])) : '-'; ?></td>
                        <td class="border-dark text-center"><?php echo !empty($p['end_date']) ? date('d/m/Y', strtotime($p['end_date'])) : '-'; ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
